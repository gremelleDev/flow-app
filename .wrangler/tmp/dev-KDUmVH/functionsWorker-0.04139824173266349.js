...
// .wrangler/tmp/bundle-fjxnxk/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-AxLY8p/functionsWorker-0.04139824173266349.mjs
...
var findOid = /* @__PURE__ */ __name2((keyData, oid, from = 0) => {
  if (from === 0) {
...
var getNamedCurve2 = /* @__PURE__ */ __name2((keyData) => {
  switch (true) {
...
var genericImport = /* @__PURE__ */ __name2(async (replace, keyFormat, pem, alg, options) => {
  let algorithm;
  let keyUsages;
...
var fromPKCS8 = /* @__PURE__ */ __name2((pem, alg, options) => {
...
__name2(importPKCS8, "importPKCS8");
...
var sign_default = /* @__PURE__ */ __name2(async (alg, key, data) => {
...
function createLocalJWKSet(jwks) {
  ...
  const localJWKSet = /* @__PURE__ */ __name2(async (protectedHeader, token2) => set.getKey(protectedHeader, token2), "localJWKSet");
...
function createRemoteJWKSet(url, options) {
  ...
  const remoteJWKSet = /* @__PURE__ */ __name2(async (protectedHeader, token2) => set.getKey(protectedHeader, token2), "remoteJWKSet");
...
async function getAccessToken(env) {
  if (token && tokenExpiry && tokenExpiry > /* @__PURE__ */ new Date()) {
  ...
  }).setProtectedHeader({ alg: "RS256" }).setIssuer(serviceAccount.client_email).setSubject(serviceAccount.client_email).setAudience(serviceAccount.token_uri).setIssuedAt().setExpirationTime("1h").sign(privateKey);
  ...
  token = tokenResponse;
  tokenExpiry = new Date((/* @__PURE__ */ new Date()).getTime() + tokenResponse.expires_in * 1e3);
  return token.access_token;
}
...
__name(lookupUserByEmail, "lookupUserByEmail");
__name2(lookupUserByEmail, "lookupUserByEmail");
async function setCustomUserClaims(uid, claims, env) {
  const accessToken = await getAccessToken(env);
  const serviceAccount = JSON.parse(env.FIREBASE_SERVICE_ACCOUNT);
...
__name(setCustomUserClaims, "setCustomUserClaims");
__name2(setCustomUserClaims, "setCustomUserClaims");
async function verifyIdToken(token2, env) {
  const serviceAccount = JSON.parse(env.FIREBASE_SERVICE_ACCOUNT);
  ...
  const { payload } = await jwtVerify(token2, JWKS, {
    issuer,
    audience
  });
  return payload;
...
async function authenticate(context) {
  ...
  try {
    const decodedToken = await verifyIdToken(token2, env);
    return decodedToken;
  } catch (error) {
...
async function getDerivedKey(env) {
  ...
  return crypto.subtle.deriveKey(
    { name: "PBKDF2", salt: new Uint8Array(16), iterations: 1e5, hash: "SHA-256" },
    keyMaterial,
...
__name(getDerivedKey, "getDerivedKey");
__name2(getDerivedKey, "getDerivedKey");
...
// .wrangler/tmp/bundle-fjxnxk/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fjxnxk/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.04139824173266349.js.map
